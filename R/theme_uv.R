...
#' @param geom_font_size default font size
#'
#' @examples \dontrun{
#' library(ggplot2)
#' library(dplyr)
...
theme_uv <- function(base_family = "IBM Plex Sans Condensed",
  ...
  set_geom_font_defaults(base_family, face = "plain", color = def_fore, size = geom_font_size)
  ...
  if (inherits(axis, "character") | axis == TRUE) {
    ret <- ret + ggplot2::theme(axis.line = ggplot2::element_line(color = def_fore, linewidth = 0.15))
