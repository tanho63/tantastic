...
<!-- badges: start -->
[![Lifecycle: experimental](https://img.shields.io/badge/lifecycle-experimental-informational?style=flat-square)](https://lifecycle.r-lib.org/articles/stages.html#experimental)
<!-- badges: end -->
...
You can install the released version of tantastic from [Tan's R-Universe page](https://tanho63.r-universe.dev) with:
...
- `theme_tantastic()` is an adapted theme from hrbrmstr/hrbrthemes's theme_modern_rc()
- `progressively()` adds some helpers for progress bars. Now superseded by `purrr::map()`.
- `use_client_tz()` and `get_client_tz()` give access to a Shiny user's timezone
